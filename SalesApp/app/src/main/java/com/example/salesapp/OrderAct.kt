...
class OrderAct : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var url="http://192.168.1.10/SalesWeb/get_temp.php?mobile="+UserInfo.mobile
        ...
        var jar= JsonArrayRequest(Request.Method.GET,url,null, Response.Listener { response ->
            for(x in 0..response.length()-1){
                list.add("Name : " +response.getJSONObject(x).getString("name")+
                        ...
                        "\nQuantity : " +response.getJSONObject(x).getInt("qty"))
            }
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        ...
        if(item?.itemId==R.id.item_cancel){
            var url="http://192.168.1.10/SalesWeb/cancel_order.php?mobile="+UserInfo.mobile
        ...
        if(item?.itemId==R.id.item_confirm){
            var url="http://192.168.1.10/SalesWeb/confirm_order.php?mobile="+UserInfo.mobile
            var rq:RequestQueue=Volley.newRequestQueue(this)
            var sr= StringRequest(Request.Method.GET,url,Response.Listener { response ->
